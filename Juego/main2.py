...
icon = pygame.image.load("Juego\\sprites\\dragonPlayer1.png")
withe = (255,255,255)
...
class Nube(pygame.sprite.Sprite):
    def __init__(self):
      super().__init__()
      self.image = pygame.image.load("Juego\\sprites\\cloud.png")
...
def Game():
    ...
    while play_game:  # mientras sea True andara este loop que es el que contiene los demas loop (es loop principal)

        if not dead:
            ######## screen.fill(GREEN)
           # screen.blit(fondo_juego, [0, 0])
            screen.blit(Background_historia.image, Background_historia.rect)
            ground.draw(screen)
